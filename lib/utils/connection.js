...
async function start(oldSocket = null, opts = { store, logger, authState }) {
  ...
  // NOTE: an earlier "watchdog" here force-reloaded the connection whenever
  // it looked idle (no connection.update event for N minutes), but that
  // idle timer only got refreshed by the 'open' event - a perfectly
  // healthy, quiet connection with no new connection.update events would
  // eventually get force-reloaded anyway. Baileys' own reconnect logic
  // (the connection === 'close' handler below, via connectionUpdate)
  // already reconnects on real disconnects with proper DisconnectReason
  // handling; this extra layer only added spurious reload cycles that
  // could race with it - and each unnecessary re-pair/reload is a chance
  // for the Signal session state to fall out of sync (a likely contributor
  // to the "Bad MAC" decrypt errors seen after repeated forced reloads).
